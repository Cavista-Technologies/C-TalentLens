...
export function ImportsPage() {
  ...
  const canResetImport = Boolean(fileName || rows.length > 0 || result || error)
  const hasPendingImport = Boolean((fileName || rows.length > 0 || error) && !result)
  const { confirmDiscard } = useUnsavedChanges(hasPendingImport)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const importResult = importType === 'requisitions' ? await importRequisitions(rows) : await importReferrals(rows)
      setResult(importResult)
      showToast(`${importResult.importedCount} ${importType} imported`)
    } catch (err) {
  ...
  async function handleSmartRecruitersSync() {
    ...
    try {
      ...
      showToast(`${syncResult.createdCount + syncResult.updatedCount} requisitions processed`)
    } catch (err) {
  ...
  return (
    <AppLayout title="Import">
      <PageContainer>
        <BackButton fallbackTo="/requisitions" onBeforeNavigate={confirmDiscard} />
        ...
        <form className="import-panel" onSubmit={handleSubmit}>
          ...
          <section className="csv-import-section" aria-label={`${getImportLabel(importType)} CSV import`}>
            ...
            <label className="import-upload-box">
              ...
              <input key={`${importType}-${fileInputKey}`} type="file" accept=".csv,text/csv" onChange={handleFileChange} />
            </label>
          ...
          <div className="form-actions import-actions">
            {canResetImport && (
              <button className="secondary-action" type="button" disabled={isSubmitting} onClick={handleResetImport}>
                Cancel
              </button>
            )}
